...
export default function EnhancedCodeChallengeModal({
  ...
  const handleRunCode = async () => {
    ...
    try {
      ...
      setTestResults(results.map((r, idx) => ({
        id: idx,
        passed: r.passed,
        input: r.input,
        expected: r.expected,
  ...
  const handleSubmit = async () => {
    ...
    if (!syntaxValid) {
      toast.error('❌ Please fix syntax errors before submitting!', { duration: 4000 });
      return;
    }
    ...
    try {
      ...
      toast.success(`✅ PASS: All ${totalTests} test cases passed! +${reward} Algo-Coins`, {
        duration: 3000,
      });
